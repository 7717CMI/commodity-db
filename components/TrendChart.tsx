...
/** Build a list of YYYY-MM month keys that appear in the dates array */
...
export default function TrendChart({ commodity, country, entity, priceType, selectedWeek }: Props) {
  ...
  const chartData = useMemo(() => {
    ...
    switch (period) {
      ...
      case 'month': {
        // Filter to the selected month only
        const monthStart = `${selectedMonth}-01`
        const monthEnd = `${selectedMonth}-31`
        return filterByRange(allPoints, monthStart, monthEnd)
      }
      case 'custom':
        return customFrom && customTo ? filterByRange(allPoints, customFrom, customTo) : allPoints
    }
  }, [allPoints, priceType, period, customFrom, customTo, selectedMonth])
  ...
  const chartKey = `${commodity}-${country}-${priceType}-${selectedWeek}-${period}-${selectedMonth}`
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          {period === 'month' && (
            <select
              ...
              className="text-xs border border-slate-200 rounded-lg px-2 py-1.5 ml-2 outline-none focus:border-teal-400 cursor-pointer"
            >
      ...
      {priceType === 'import' && (
        <div className="mb-4">
          <span className="text-xs font-medium px-3 py-1.5 rounded-full bg-teal-50 text-teal-700 border border-teal-100">
            {`Week ${selectedWeek + 1}`} — 7 daily prices
          </span>
        </div>
      )}
